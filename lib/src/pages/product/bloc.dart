...
class ProductBloc extends CachedBloc<ProductEvent, ProductState> {
  ProductBloc([String globalKey = '']) : super(() => ProductState(), globalKey) {
  ...
  Future<void> _loadData({void Function(String message)? onError}) async {
    add(StateChangedEvent(ProductState(status: Status.loading)));
    await Future.delayed(const Duration(milliseconds: 500));
    //TODO load data
    if(isClosed){
      return;
    }
    add(StateChangedEvent(ProductState(status: Status.success)));
  }
}
